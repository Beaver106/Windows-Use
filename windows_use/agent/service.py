...
class Agent:
    def __init__(self,mode:Literal["flash","full"]="full",instructions:list[str]=[],browser:Browser=Browser.EDGE, use_annotation:bool=False, llm: BaseChatLLM=None,max_consecutive_failures:int=3,max_steps:int=25,use_vision:bool=False,auto_minimize:bool=False,experimental:bool=False,save_screenshots:bool=False,save_screenshot_path:str="screenshots"):
        '''
        ...
        Args:
            ...
            save_screenshots (bool): Whether to save screenshots to disk. Defaults to False.
            save_screenshot_path (str): Directory path to save screenshots. Defaults to "screenshots".
        '''
        ...
        self.save_screenshot_path = Path(save_screenshot_path)
        if self.save_screenshots:
            self.save_screenshot_path.mkdir(parents=True, exist_ok=True)
        self.llm = llm
    ...
    def _save_screenshot_if_enabled(self, desktop_state, step: int = 0, last_action: dict = None):
        """Save screenshot if save_screenshots is enabled, with optional click annotation."""
        if self.save_screenshots and desktop_state.screenshot:
            filename = f"screenshot_step_{step:03d}.png"
            filepath = self.save_screenshot_path / filename
            
            # Annotate click location if action was a click/type/scroll
            screenshot = desktop_state.screenshot.copy()
            if last_action and last_action.get('name') in ['click_tool', 'type_tool', 'scroll_tool', 'move_tool']:
                params = last_action.get('params', {})
                loc = params.get('loc')
                if loc and len(loc) == 2:
                    screenshot = self._annotate_action_on_screenshot(screenshot, loc, last_action['name'])
            
            screenshot.save(filepath)
            logger.info(f"[Screenshot] Saved to: {filepath}")
    
    def _annotate_action_on_screenshot(self, screenshot, loc, action_name):
        """Draw a marker on the screenshot at the action location."""
        ...
        color = color_map.get(action_name, 'red')
        ...
        bbox = draw.textbbox((x + 25, y - 10), text, font=font)
        draw.rectangle(bbox, fill='white', outline=color)
        draw.text((x + 25, y - 10), text, fill=color, font=font)
        ...
        try:
            ...
            with (self.desktop.auto_minimize() if self.auto_minimize else nullcontext()):
                ...
                with self.watchdog:
                    ...
                    self._save_screenshot_if_enabled(desktop_state, step=0)
                    ...
                    while step < self.max_steps:
                        ...
                        if action_name.lower().startswith('done'):
                            ...
                            desktop_state = self.desktop.get_state(use_annotation=self.use_annotation,use_vision=self.use_vision)
                            # Pass action info for annotation
                            last_action = {'name': action_name, 'params': params}
                            self._save_screenshot_if_enabled(desktop_state, step=step, last_action=last_action)
                            human_prompt = Prompt.observation_prompt(query=query, step=step, max_steps=self.max_steps,
